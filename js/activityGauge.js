function activityGaugeDefaultSettings() {
    return {
...
function activityGauge(elementId, value, config) {
    if(config == null) config = activityGaugeDefaultSettings();
    ...
    var locationY = parseInt(gauge.style("height"))/3 - 0.9*radius;
    ...
    function GaugeUpdater(){
            ...
            var textTween = function() {
                var node = this;
                var i = d3.interpolate(node.textContent, parseFloat(value).toFixed(1));
                return function(t) {
                    node.textContent = textRounderUpdater(i(t)) + percentText;
                };
            };
            text1.transition()
                .duration(config.waveRiseTime)
